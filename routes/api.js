...
const listkey = ["xZiyyApis","ApiKey","AldiFauzi"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'AldiFauzi') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'AldiFauzi')  return res.json(loghandler.invalidKey)
